...
class VisionApi(object):
    ...
    #   get string variable
    # ---------------------------------------------------------------------------------------------
    def get_variable_string(self, varname, gap_time=(0, 2), pattern=".*"):
        time_start = time() + gap_time[0]
        time_end   = time() + gap_time[1] 
        while time() < time_end:
            sleep(0.1)
            if varname in self.__data_base:
                if self.__data_base[varname][1] <= time_start:
                    continue
                result = match(pattern, str(self.__data_base[varname][0]))
                if not result:
                    continue                    
                return self.__data_base[varname][0]
        raise ValueError('not found(var=%s)'%(varname))
